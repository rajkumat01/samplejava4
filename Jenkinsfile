...
pipeline {
    ...
    stages {
        stage('Clone repository') {               
           steps{
                // checkout scm
                git branch: 'master', url: 'https://github.com/rajkumat01/samplejava4'
           }
        }     
        stage('Upload JSON'){
            steps{
    ...
    post {
        success {
            echo 'Run E2ESamplePipeLine4_1!'
            build job: 'E2ESamplePipeLine4_1', parameters: [string(name: 'MY_PARAM', value: 'value from Build pipeline')]
        }
